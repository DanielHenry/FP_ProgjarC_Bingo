package DesignSource;

import javax.swing.JList;

/**
 *
 * @author fikri
 */
public class GameRoom extends javax.swing.JFrame {

    /**
     * Creates new form ClientFrame
     */
    private int x=1;
    private boolean buttonEnable = false;
    private int bingo_size;
    
    public void setActive()
    {
    	this.buttonEnable = true;
    }
    public GameRoom(int bingo_size) 
    {
    	this.bingo_size = bingo_size;
        initComponents();
    }

    /**
     * This method is called from within the constructor to initialize the form.
     * WARNING: Do NOT modify this code. The content of this method is always
     * regenerated by the Form Editor.
     */
    @SuppressWarnings("unchecked")
    // <editor-fold defaultstate="collapsed" desc="Generated Code">//GEN-BEGIN:initComponents
    private void initComponents() {

        jScrollPane1 = new javax.swing.JScrollPane();
        jListPlayer = new javax.swing.JList();
        jLabelPlayer = new javax.swing.JLabel();
        jScrollPane3 = new javax.swing.JScrollPane();
        jTextArea1 = new javax.swing.JTextArea();
        jTextFiledWispName = new javax.swing.JTextField();
        jTextFieldChat = new javax.swing.JTextField();
        jButtonSendChat = new javax.swing.JButton();
        d9 = new javax.swing.JButton();
        i9 = new javax.swing.JButton();
        h9 = new javax.swing.JButton();
        c6 = new javax.swing.JButton();
        b6 = new javax.swing.JButton();
        e6 = new javax.swing.JButton();
        d6 = new javax.swing.JButton();
        g5 = new javax.swing.JButton();
        a6 = new javax.swing.JButton();
        g6 = new javax.swing.JButton();
        g1 = new javax.swing.JButton();
        g2 = new javax.swing.JButton();
        g3 = new javax.swing.JButton();
        g4 = new javax.swing.JButton();
        c9 = new javax.swing.JButton();
        a9 = new javax.swing.JButton();
        b9 = new javax.swing.JButton();
        i2 = new javax.swing.JButton();
        i3 = new javax.swing.JButton();
        i4 = new javax.swing.JButton();
        i1 = new javax.swing.JButton();
        e2 = new javax.swing.JButton();
        e3 = new javax.swing.JButton();
        e4 = new javax.swing.JButton();
        e5 = new javax.swing.JButton();
        a5 = new javax.swing.JButton();
        f1 = new javax.swing.JButton();
        f2 = new javax.swing.JButton();
        f3 = new javax.swing.JButton();
        a2 = new javax.swing.JButton();
        a1 = new javax.swing.JButton();
        f4 = new javax.swing.JButton();
        b2 = new javax.swing.JButton();
        b1 = new javax.swing.JButton();
        f6 = new javax.swing.JButton();
        a4 = new javax.swing.JButton();
        f5 = new javax.swing.JButton();
        a3 = new javax.swing.JButton();
        i8 = new javax.swing.JButton();
        i7 = new javax.swing.JButton();
        b4 = new javax.swing.JButton();
        i6 = new javax.swing.JButton();
        b3 = new javax.swing.JButton();
        i5 = new javax.swing.JButton();
        e9 = new javax.swing.JButton();
        f9 = new javax.swing.JButton();
        g9 = new javax.swing.JButton();
        g8 = new javax.swing.JButton();
        h8 = new javax.swing.JButton();
        h7 = new javax.swing.JButton();
        h6 = new javax.swing.JButton();
        d4 = new javax.swing.JButton();
        h5 = new javax.swing.JButton();
        d2 = new javax.swing.JButton();
        d3 = new javax.swing.JButton();
        c5 = new javax.swing.JButton();
        d1 = new javax.swing.JButton();
        c3 = new javax.swing.JButton();
        c4 = new javax.swing.JButton();
        c1 = new javax.swing.JButton();
        c2 = new javax.swing.JButton();
        e1 = new javax.swing.JButton();
        d5 = new javax.swing.JButton();
        c8 = new javax.swing.JButton();
        d8 = new javax.swing.JButton();
        a8 = new javax.swing.JButton();
        b8 = new javax.swing.JButton();
        f8 = new javax.swing.JButton();
        e8 = new javax.swing.JButton();
        f7 = new javax.swing.JButton();
        e7 = new javax.swing.JButton();
        b7 = new javax.swing.JButton();
        a7 = new javax.swing.JButton();
        d7 = new javax.swing.JButton();
        c7 = new javax.swing.JButton();
        b5 = new javax.swing.JButton();
        h1 = new javax.swing.JButton();
        h2 = new javax.swing.JButton();
        h3 = new javax.swing.JButton();
        h4 = new javax.swing.JButton();
        g7 = new javax.swing.JButton();

        setDefaultCloseOperation(javax.swing.WindowConstants.EXIT_ON_CLOSE);
        setMinimumSize(new java.awt.Dimension(840, 640));
        setResizable(false);

        jListPlayer.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        jListPlayer.setModel(new javax.swing.AbstractListModel() {
            String[] strings = { "Player 1", "Player 2", "Player 3", "Player 4", "Player 5" };
            public int getSize() { return strings.length; }
            public Object getElementAt(int i) { return strings[i]; }
        });
        jListPlayer.setSelectionMode(javax.swing.ListSelectionModel.SINGLE_SELECTION);
        jListPlayer.setDebugGraphicsOptions(javax.swing.DebugGraphics.BUFFERED_OPTION);
        jListPlayer.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                jListPlayerMouseClicked(evt);
            }
            public void mousePressed(java.awt.event.MouseEvent evt) {
                jListPlayerMousePressed(evt);
            }
        });
        jScrollPane1.setViewportView(jListPlayer);

        jLabelPlayer.setFont(new java.awt.Font("Tahoma", 1, 24)); // NOI18N
        jLabelPlayer.setText("Player List");
        jLabelPlayer.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                jLabelPlayerMouseClicked(evt);
            }
            public void mousePressed(java.awt.event.MouseEvent evt) {
                jLabelPlayerMousePressed(evt);
            }
        });

        jTextArea1.setColumns(20);
        jTextArea1.setRows(5);
        jScrollPane3.setViewportView(jTextArea1);

        jTextFiledWispName.setText("Whisper");
        jTextFiledWispName.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jTextFiledWispNameActionPerformed(evt);
            }
        });

        jTextFieldChat.setText("Chat");

        jButtonSendChat.setText("Send");

        d9.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        d9.setText("-");
        d9.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) 
            {
            	d9ActionPerformed(evt);
            }
        });

        i9.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        i9.setText("-");
        i9.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                i9ActionPerformed(evt);
            }
        });

        h9.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        h9.setText("-");
        h9.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                h9ActionPerformed(evt);
            }
        });

        c6.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        c6.setText("-");
        c6.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                c6ActionPerformed(evt);
            }
        });

        b6.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        b6.setText("-");
        b6.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                b6ActionPerformed(evt);
            }
        });

        e6.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        e6.setText("-");
        e6.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                e6ActionPerformed(evt);
            }
        });

        d6.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        d6.setText("-");
        d6.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                d6ActionPerformed(evt);
            }
        });

        g5.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        g5.setText("-");
        g5.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                g5ActionPerformed(evt);
            }
        });

        a6.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        a6.setText("-");
        a6.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                a6ActionPerformed(evt);
            }
        });

        g6.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        g6.setText("-");
        g6.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                g6ActionPerformed(evt);
            }
        });

        g1.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        g1.setText("-");
        g1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                g1ActionPerformed(evt);
            }
        });

        g2.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        g2.setText("-");
        g2.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                g2ActionPerformed(evt);
            }
        });

        g3.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        g3.setText("-");
        g3.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                g3ActionPerformed(evt);
            }
        });

        g4.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        g4.setText("-");
        g4.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                g4ActionPerformed(evt);
            }
        });

        c9.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        c9.setText("-");
        c9.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                c9ActionPerformed(evt);
            }
        });

        a9.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        a9.setText("-");
        a9.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                a9ActionPerformed(evt);
            }
        });

        b9.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        b9.setText("-");
        b9.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                b9ActionPerformed(evt);
            }
        });

        i2.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        i2.setText("-");
        i2.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                i2ActionPerformed(evt);
            }
        });

        i3.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        i3.setText("-");
        i3.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                i3ActionPerformed(evt);
            }
        });

        i4.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        i4.setText("-");
        i4.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                i4ActionPerformed(evt);
            }
        });

        i1.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        i1.setText("-");
        i1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                i1ActionPerformed(evt);
            }
        });

        e2.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        e2.setText("-");
        e2.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                e2ActionPerformed(evt);
            }
        });

        e3.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        e3.setText("-");
        e3.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                e3ActionPerformed(evt);
            }
        });

        e4.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        e4.setText("-");
        e4.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                e4ActionPerformed(evt);
            }
        });

        e5.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        e5.setText("-");
        e5.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                e5ActionPerformed(evt);
            }
        });

        a5.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        a5.setText("-");
        a5.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                a5ActionPerformed(evt);
            }
        });

        f1.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        f1.setText("-");
        f1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                f1ActionPerformed(evt);
            }
        });

        f2.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        f2.setText("-");
        f2.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                f2ActionPerformed(evt);
            }
        });

        f3.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        f3.setText("-");
        f3.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                f3ActionPerformed(evt);
            }
        });

        a2.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        a2.setText("-");
        a2.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                a2ActionPerformed(evt);
            }
        });

        a1.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        a1.setText("10");
        a1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                a1ActionPerformed(evt);
            }
        });

        f4.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        f4.setText("-");
        f4.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                f4ActionPerformed(evt);
            }
        });

        b2.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        b2.setText("-");
        b2.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                b2ActionPerformed(evt);
            }
        });

        b1.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        b1.setText("-");
        b1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                b1ActionPerformed(evt);
            }
        });

        f6.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        f6.setText("-");
        f6.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                f6ActionPerformed(evt);
            }
        });

        a4.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        a4.setText("-");
        a4.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                a4ActionPerformed(evt);
            }
        });

        f5.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        f5.setText("-");
        f5.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                f5ActionPerformed(evt);
            }
        });

        a3.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        a3.setText("-");
        a3.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                a3ActionPerformed(evt);
            }
        });

        i8.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        i8.setText("-");
        i8.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                i8ActionPerformed(evt);
            }
        });

        i7.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        i7.setText("-");
        i7.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                i7ActionPerformed(evt);
            }
        });

        b4.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        b4.setText("-");
        b4.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                b4ActionPerformed(evt);
            }
        });

        i6.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        i6.setText("-");
        i6.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                i6ActionPerformed(evt);
            }
        });

        b3.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        b3.setText("-");
        b3.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                b3ActionPerformed(evt);
            }
        });

        i5.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        i5.setText("-");
        i5.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                i5ActionPerformed(evt);
            }
        });

        e9.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        e9.setText("-");
        e9.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                e9ActionPerformed(evt);
            }
        });

        f9.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        f9.setText("-");
        f9.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                f9ActionPerformed(evt);
            }
        });

        g9.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        g9.setText("-");
        g9.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                g9ActionPerformed(evt);
            }
        });

        g8.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        g8.setText("-");
        g8.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                g8ActionPerformed(evt);
            }
        });

        h8.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        h8.setText("-");
        h8.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                h8ActionPerformed(evt);
            }
        });

        h7.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        h7.setText("-");
        h7.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                h7ActionPerformed(evt);
            }
        });

        h6.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        h6.setText("-");
        h6.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                h6ActionPerformed(evt);
            }
        });

        d4.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        d4.setText("-");
        d4.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                d4ActionPerformed(evt);
            }
        });

        h5.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        h5.setText("-");
        h5.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                h5ActionPerformed(evt);
            }
        });

        d2.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        d2.setText("-");
        d2.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                d2ActionPerformed(evt);
            }
        });

        d3.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        d3.setText("-");
        d3.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                d3ActionPerformed(evt);
            }
        });

        c5.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        c5.setText("-");
        c5.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                c5ActionPerformed(evt);
            }
        });

        d1.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        d1.setText("-");
        d1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                d1ActionPerformed(evt);
            }
        });

        c3.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        c3.setText("-");
        c3.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                c3ActionPerformed(evt);
            }
        });

        c4.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        c4.setText("-");
        c4.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                c4ActionPerformed(evt);
            }
        });

        c1.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        c1.setText("-");
        c1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                c1ActionPerformed(evt);
            }
        });

        c2.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        c2.setText("-");
        c2.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                c2ActionPerformed(evt);
            }
        });

        e1.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        e1.setText("-");
        e1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                e1ActionPerformed(evt);
            }
        });

        d5.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        d5.setText("-");
        d5.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                d5ActionPerformed(evt);
            }
        });

        c8.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        c8.setText("-");
        c8.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                c8ActionPerformed(evt);
            }
        });

        d8.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        d8.setText("-");
        d8.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                d8ActionPerformed(evt);
            }
        });

        a8.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        a8.setText("-");
        a8.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                a8ActionPerformed(evt);
            }
        });

        b8.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        b8.setText("-");
        b8.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                b8ActionPerformed(evt);
            }
        });

        f8.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        f8.setText("-");
        f8.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                f8ActionPerformed(evt);
            }
        });

        e8.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        e8.setText("-");
        e8.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                e8ActionPerformed(evt);
            }
        });

        f7.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        f7.setText("-");
        f7.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                f7ActionPerformed(evt);
            }
        });

        e7.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        e7.setText("-");
        e7.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                e7ActionPerformed(evt);
            }
        });

        b7.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        b7.setText("-");
        b7.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                b7ActionPerformed(evt);
            }
        });

        a7.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        a7.setText("-");
        a7.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                a7ActionPerformed(evt);
            }
        });

        d7.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        d7.setText("-");
        d7.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                d7ActionPerformed(evt);
            }
        });

        c7.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        c7.setText("-");
        c7.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                c7ActionPerformed(evt);
            }
        });

        b5.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        b5.setText("-");
        b5.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                b5ActionPerformed(evt);
            }
        });

        h1.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        h1.setText("-");
        h1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                h1ActionPerformed(evt);
            }
        });

        h2.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        h2.setText("-");
        h2.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                h2ActionPerformed(evt);
            }
        });

        h3.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        h3.setText("-");
        h3.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                h3ActionPerformed(evt);
            }
        });

        h4.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        h4.setText("-");
        h4.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                h4ActionPerformed(evt);
            }
        });

        g7.setFont(new java.awt.Font("Tahoma", 0, 14)); // NOI18N
        g7.setText("-");
        g7.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                g7ActionPerformed(evt);
            }
        });

        javax.swing.GroupLayout layout = new javax.swing.GroupLayout(getContentPane());
        getContentPane().setLayout(layout);
        layout.setHorizontalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(layout.createSequentialGroup()
                .addContainerGap()
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(jTextFiledWispName, javax.swing.GroupLayout.PREFERRED_SIZE, 88, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(jTextFieldChat, javax.swing.GroupLayout.PREFERRED_SIZE, 640, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(jButtonSendChat, javax.swing.GroupLayout.DEFAULT_SIZE, 90, Short.MAX_VALUE))
                    .addGroup(layout.createSequentialGroup()
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(layout.createSequentialGroup()
                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    .addGroup(layout.createSequentialGroup()
                                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                            .addGroup(layout.createSequentialGroup()
                                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                                    .addGroup(layout.createSequentialGroup()
                                                        .addComponent(a1, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(a2, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(a3, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(a4, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(a5, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE))
                                                    .addGroup(layout.createSequentialGroup()
                                                        .addComponent(b1, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(b2, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(b3, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(b4, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(b5, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE))
                                                    .addGroup(layout.createSequentialGroup()
                                                        .addComponent(c1, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(c2, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(c3, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(c4, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(c5, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE))
                                                    .addGroup(layout.createSequentialGroup()
                                                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                                            .addGroup(layout.createSequentialGroup()
                                                                .addComponent(d1, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                                .addComponent(d2, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE))
                                                            .addGroup(layout.createSequentialGroup()
                                                                .addComponent(e1, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                                .addComponent(e2, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)))
                                                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                                            .addGroup(layout.createSequentialGroup()
                                                                .addGap(6, 6, 6)
                                                                .addComponent(e3, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                                .addComponent(e4, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                                .addComponent(e5, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE))
                                                            .addGroup(layout.createSequentialGroup()
                                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                                .addComponent(d3, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                                .addComponent(d4, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                                .addComponent(d5, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE))))
                                                    .addGroup(layout.createSequentialGroup()
                                                        .addComponent(f1, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(f2, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(f3, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(f4, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                        .addComponent(f5, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)))
                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                                    .addComponent(a6, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                    .addComponent(b6, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                    .addComponent(c6, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                    .addComponent(e6, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                    .addComponent(d6, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                    .addComponent(f6, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)))
                                            .addGroup(layout.createSequentialGroup()
                                                .addComponent(g1, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                .addComponent(g2, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                .addComponent(g3, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                .addComponent(g4, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                .addComponent(g5, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                                .addComponent(g6, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)))
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                            .addComponent(a7, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(b7, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(c7, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(e7, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(d7, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(f7, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(g7, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)))
                                    .addGroup(layout.createSequentialGroup()
                                        .addComponent(h1, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addComponent(h2, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addComponent(h3, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addComponent(h4, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addComponent(h5, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addComponent(h6, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addComponent(h7, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)))
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    .addComponent(a8, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addComponent(b8, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addComponent(c8, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addComponent(e8, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addComponent(d8, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addComponent(f8, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addComponent(g8, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addComponent(h8, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)))
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(i1, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(i2, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(i3, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(i4, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(i5, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(i6, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(i7, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(i8, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)))
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(a9, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(b9, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(c9, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(e9, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(d9, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(f9, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(g9, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(h9, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(i9, javax.swing.GroupLayout.PREFERRED_SIZE, 60, javax.swing.GroupLayout.PREFERRED_SIZE))
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(jScrollPane1, javax.swing.GroupLayout.PREFERRED_SIZE, 156, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, layout.createSequentialGroup()
                                .addComponent(jLabelPlayer)
                                .addGap(14, 14, 14))))
                    .addComponent(jScrollPane3))
                .addContainerGap())
        );
        layout.setVerticalGroup(
            layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
            .addGroup(layout.createSequentialGroup()
                .addContainerGap()
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(jLabelPlayer)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                        .addComponent(jScrollPane1, javax.swing.GroupLayout.PREFERRED_SIZE, 360, javax.swing.GroupLayout.PREFERRED_SIZE))
                    .addGroup(layout.createSequentialGroup()
                        .addComponent(a9, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(b9, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(c9, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(d9, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(e9, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(f9, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(g9, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(h9, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(i9, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE))
                    .addGroup(layout.createSequentialGroup()
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(a8, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(b8, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(c8, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(d8, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(e8, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(f8, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(g8, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE))
                            .addGroup(layout.createSequentialGroup()
                                .addComponent(a7, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(b7, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(c7, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(d7, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(e7, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(f7, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addComponent(g7, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE))
                            .addGroup(layout.createSequentialGroup()
                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    .addGroup(layout.createSequentialGroup()
                                        .addComponent(a6, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addComponent(b6, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addComponent(c6, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addComponent(d6, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addComponent(e6, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addComponent(f6, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE))
                                    .addGroup(layout.createSequentialGroup()
                                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                            .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                                                .addComponent(a2, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                .addComponent(a3, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                .addComponent(a4, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                                .addComponent(a5, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE))
                                            .addComponent(a1, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE))
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                                            .addComponent(b1, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(b2, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(b3, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(b4, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(b5, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE))
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                                            .addComponent(c1, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(c2, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(c4, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(c3, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(c5, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE))
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                                            .addComponent(d1, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(d2, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(d3, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(d4, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(d5, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE))
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                                            .addComponent(e1, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(e2, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(e3, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(e4, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(e5, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE))
                                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                                            .addComponent(f1, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(f2, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(f3, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(f4, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                            .addComponent(f5, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE))))
                                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                    .addComponent(g6, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                    .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                                        .addComponent(g1, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addComponent(g2, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addComponent(g3, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addComponent(g4, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                        .addComponent(g5, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)))))
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(h8, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(h7, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(h6, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                                .addComponent(h1, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addComponent(h2, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addComponent(h3, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addComponent(h4, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addComponent(h5, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)))
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            .addComponent(i8, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(i7, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(i6, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                                .addComponent(i1, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addComponent(i2, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addComponent(i3, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addComponent(i4, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)
                                .addComponent(i5, javax.swing.GroupLayout.PREFERRED_SIZE, 40, javax.swing.GroupLayout.PREFERRED_SIZE)))))
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                .addComponent(jScrollPane3, javax.swing.GroupLayout.PREFERRED_SIZE, 160, javax.swing.GroupLayout.PREFERRED_SIZE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.BASELINE)
                    .addComponent(jTextFiledWispName, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(jTextFieldChat, javax.swing.GroupLayout.PREFERRED_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.PREFERRED_SIZE)
                    .addComponent(jButtonSendChat))
                .addContainerGap(21, Short.MAX_VALUE))
        );

        pack();
    }// </editor-fold>//GEN-END:initComponents

    private void jListPlayerMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jListPlayerMousePressed
        // TODO add your handling code here:
    }//GEN-LAST:event_jListPlayerMousePressed

    private void jListPlayerMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jListPlayerMouseClicked
        // TODO add your handling code here:
    }//GEN-LAST:event_jListPlayerMouseClicked

    private void jLabelPlayerMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabelPlayerMouseClicked
        // TODO add your handling code here:
        jListPlayer.clearSelection();
        jListPlayer.repaint();
    }//GEN-LAST:event_jLabelPlayerMouseClicked

    
    private void jTextFiledWispNameActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jTextFiledWispNameActionPerformed
        // TODO add your handling code here:
    }//GEN-LAST:event_jTextFiledWispNameActionPerformed

    private void jLabelPlayerMousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jLabelPlayerMousePressed
        // TODO add your handling code here:
    }//GEN-LAST:event_jLabelPlayerMousePressed

    private void d9ActionPerformed(java.awt.event.ActionEvent evt) 
    {//GEN-FIRST:event_d9ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (d9.getText().equals("-"))
            {
                d9.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    	
    }//GEN-LAST:event_d9ActionPerformed

    private void i9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_i9ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (i9.getText().equals("-"))
            {
                i9.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_i9ActionPerformed

    private void h9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_h9ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (h9.getText().equals("-"))
            {
                h9.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_h9ActionPerformed

    private void c6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_c6ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (c6.getText().equals("-"))
            {
                c6.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_c6ActionPerformed

    private void b6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b6ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (b6.getText().equals("-"))
            {
                b6.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_b6ActionPerformed

    private void e6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_e6ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (e6.getText().equals("-"))
            {
                e6.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_e6ActionPerformed

    private void d6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_d6ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (d6.getText().equals("-"))
            {
                d6.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_d6ActionPerformed

    private void g5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_g5ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (g5.getText().equals("-"))
            {
                g5.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_g5ActionPerformed

    private void a6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_a6ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (a6.getText().equals("-"))
            {
                a6.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_a6ActionPerformed

    private void g6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_g6ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (g6.getText().equals("-"))
            {
                g6.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_g6ActionPerformed

    private void g1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_g1ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (g1.getText().equals("-"))
            {
                g1.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_g1ActionPerformed

    private void g2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_g2ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (g2.getText().equals("-"))
            {
                g2.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_g2ActionPerformed

    private void g3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_g3ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (g3.getText().equals("-"))
            {
                g3.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_g3ActionPerformed

    private void g4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_g4ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (g4.getText().equals("-"))
            {
                g4.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_g4ActionPerformed

    private void c9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_c9ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (c9.getText().equals("-"))
            {
                c9.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_c9ActionPerformed

    private void a9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_a9ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (a9.getText().equals("-"))
            {
                a9.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_a9ActionPerformed

    private void b9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b9ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (b9.getText().equals("-"))
            {
                b9.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_b9ActionPerformed

    private void i2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_i2ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (i2.getText().equals("-"))
            {
                i2.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_i2ActionPerformed

    private void i3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_i3ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (i3.getText().equals("-"))
            {
                i3.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}    }//GEN-LAST:event_i3ActionPerformed

    private void i4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_i4ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (i4.getText().equals("-"))
            {
                i4.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_i4ActionPerformed

    private void i1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_i1ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (i1.getText().equals("-"))
            {
                i1.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_i1ActionPerformed

    private void e2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_e2ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (e2.getText().equals("-"))
            {
                e2.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_e2ActionPerformed

    private void e3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_e3ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (e3.getText().equals("-"))
            {
                e3.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_e3ActionPerformed

    private void e4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_e4ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (e4.getText().equals("-"))
            {
                e4.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_e4ActionPerformed

    private void e5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_e5ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (e5.getText().equals("-"))
            {
                e5.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_e5ActionPerformed

    private void a5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_a5ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (a5.getText().equals("-"))
            {
                a5.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_a5ActionPerformed

    private void f1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_f1ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (f1.getText().equals("-"))
            {
                f1.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_f1ActionPerformed

    private void f2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_f2ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (f2.getText().equals("-"))
            {
                f2.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_f2ActionPerformed

    private void f3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_f3ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (f3.getText().equals("-"))
            {
                f3.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_f3ActionPerformed

    private void a2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_a2ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (a2.getText().equals("-"))
            {
                a2.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_a2ActionPerformed

    private void a1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_a1ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (a1.getText().equals("-"))
            {
                a1.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
        //nanti isi else a1.getText().equals("object angka yang dikirim acak dari server") untuk ngecek, dan langsung berubah jadi variabel lain.
    }//GEN-LAST:event_a1ActionPerformed

    private void f4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_f4ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (f4.getText().equals("-"))
            {
                f4.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_f4ActionPerformed

    private void b2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b2ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (b2.getText().equals("-"))
            {
                b2.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_b2ActionPerformed

    private void b1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b1ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (b1.getText().equals("-"))
            {
                b1.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_b1ActionPerformed

    private void f6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_f6ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (f6.getText().equals("-"))
            {
                f6.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}    
    }//GEN-LAST:event_f6ActionPerformed

    private void a4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_a4ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (a4.getText().equals("-"))
            {
                a4.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_a4ActionPerformed

    private void f5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_f5ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (f5.getText().equals("-"))
            {
                f5.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_f5ActionPerformed

    private void a3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_a3ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (a3.getText().equals("-"))
            {
                a3.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_a3ActionPerformed

    private void i8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_i8ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (i8.getText().equals("-"))
            {
                i8.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_i8ActionPerformed

    private void i7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_i7ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (i7.getText().equals("-"))
            {
                i7.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_i7ActionPerformed

    private void b4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b4ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (b4.getText().equals("-"))
            {
                b4.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_b4ActionPerformed

    private void i6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_i6ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (i6.getText().equals("-"))
            {
                i6.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_i6ActionPerformed

    private void b3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b3ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (b3.getText().equals("-"))
            {
                b3.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_b3ActionPerformed

    private void i5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_i5ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (i5.getText().equals("-"))
            {
                i5.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_i5ActionPerformed

    private void e9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_e9ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (e9.getText().equals("-"))
            {
                e9.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_e9ActionPerformed

    private void f9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_f9ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (f9.getText().equals("-"))
            {
                f9.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_f9ActionPerformed

    private void g9ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_g9ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (g9.getText().equals("-"))
            {
                g9.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_g9ActionPerformed

    private void g8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_g8ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (g8.getText().equals("-"))
            {
                g8.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_g8ActionPerformed

    private void h8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_h8ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (h8.getText().equals("-"))
            {
                h8.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_h8ActionPerformed

    private void h7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_h7ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (h7.getText().equals("-"))
            {
                h7.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_h7ActionPerformed

    private void h6ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_h6ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (h6.getText().equals("-"))
            {
                h6.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_h6ActionPerformed

    private void d4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_d4ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (d4.getText().equals("-"))
            {
                d4.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_d4ActionPerformed

    private void h5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_h5ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (h5.getText().equals("-"))
            {
                h5.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_h5ActionPerformed

    private void d2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_d2ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (d2.getText().equals("-"))
            {
                d2.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_d2ActionPerformed

    private void d3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_d3ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (d3.getText().equals("-"))
            {
                d3.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_d3ActionPerformed

    private void c5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_c5ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (c5.getText().equals("-"))
            {
                c5.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_c5ActionPerformed

    private void d1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_d1ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (d1.getText().equals("-"))
            {
                d1.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_d1ActionPerformed

    private void c3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_c3ActionPerformed
        if(c3.getText().equals("-")){
            c3.setText(Integer.toString(x));
            x++;
        }
    }//GEN-LAST:event_c3ActionPerformed

    private void c4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_c4ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (c4.getText().equals("-"))
            {
                c4.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_c4ActionPerformed

    private void c1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_c1ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (c1.getText().equals("-"))
            {
                c1.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_c1ActionPerformed

    private void c2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_c2ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (c2.getText().equals("-"))
            {
                c2.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_c2ActionPerformed

    private void e1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_e1ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (e1.getText().equals("-"))
            {
                e1.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_e1ActionPerformed

    private void d5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_d5ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (d5.getText().equals("-"))
            {
                d5.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_d5ActionPerformed

    private void c8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_c8ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (c8.getText().equals("-"))
            {
                c8.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_c8ActionPerformed

    private void d8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_d8ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (d8.getText().equals("-"))
            {
                d8.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_d8ActionPerformed

    private void a8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_a8ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (a8.getText().equals("-"))
            {
                a8.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_a8ActionPerformed

    private void b8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b8ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (b8.getText().equals("-"))
            {
                b8.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_b8ActionPerformed

    private void f8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_f8ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (f8.getText().equals("-"))
            {
                f8.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_f8ActionPerformed

    private void e8ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_e8ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (e8.getText().equals("-"))
            {
                e8.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_e8ActionPerformed

    private void f7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_f7ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (f7.getText().equals("-"))
            {
                f7.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_f7ActionPerformed

    private void e7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_e7ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (e7.getText().equals("-"))
            {
                e7.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_e7ActionPerformed

    private void b7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b7ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (b7.getText().equals("-"))
            {
                b7.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_b7ActionPerformed

    private void a7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_a7ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (a7.getText().equals("-"))
            {
                a7.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_a7ActionPerformed

    private void d7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_d7ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (d7.getText().equals("-"))
            {
                d7.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_d7ActionPerformed

    private void c7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_c7ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (c7.getText().equals("-"))
            {
                c7.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_c7ActionPerformed

    private void b5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_b5ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (b5.getText().equals("-"))
            {
                b5.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_b5ActionPerformed

    private void h1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_h1ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (h1.getText().equals("-"))
            {
                h1.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_h1ActionPerformed

    private void h2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_h2ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (h2.getText().equals("-"))
            {
                h2.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_h2ActionPerformed

    private void h3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_h3ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (h3.getText().equals("-"))
            {
                h3.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_h3ActionPerformed

    private void h4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_h4ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (h4.getText().equals("-"))
            {
                h4.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_h4ActionPerformed

    private void g7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_g7ActionPerformed
    	if (x!=(this.bingo_size * this.bingo_size))
    	{
    		if (g7.getText().equals("-"))
            {
                g7.setText(Integer.toString(x));
                x++;
            }
    	}	
    	else if (this.buttonEnable==true)
    	{
    		/*kirim angka ke server*/;
    		this.buttonEnable = false;
    	}
    }//GEN-LAST:event_g7ActionPerformed

    /**
     * @param args the command line arguments
     */

    // Variables declaration - do not modify//GEN-BEGIN:variables
    private javax.swing.JButton a1;
    private javax.swing.JButton a2;
    private javax.swing.JButton a3;
    private javax.swing.JButton a4;
    private javax.swing.JButton a5;
    private javax.swing.JButton a6;
    private javax.swing.JButton a7;
    private javax.swing.JButton a8;
    private javax.swing.JButton a9;
    private javax.swing.JButton b1;
    private javax.swing.JButton b2;
    private javax.swing.JButton b3;
    private javax.swing.JButton b4;
    private javax.swing.JButton b5;
    private javax.swing.JButton b6;
    private javax.swing.JButton b7;
    private javax.swing.JButton b8;
    private javax.swing.JButton b9;
    private javax.swing.JButton c1;
    private javax.swing.JButton c2;
    private javax.swing.JButton c3;
    private javax.swing.JButton c4;
    private javax.swing.JButton c5;
    private javax.swing.JButton c6;
    private javax.swing.JButton c7;
    private javax.swing.JButton c8;
    private javax.swing.JButton c9;
    private javax.swing.JButton d1;
    private javax.swing.JButton d2;
    private javax.swing.JButton d3;
    private javax.swing.JButton d4;
    private javax.swing.JButton d5;
    private javax.swing.JButton d6;
    private javax.swing.JButton d7;
    private javax.swing.JButton d8;
    private javax.swing.JButton d9;
    private javax.swing.JButton e1;
    private javax.swing.JButton e2;
    private javax.swing.JButton e3;
    private javax.swing.JButton e4;
    private javax.swing.JButton e5;
    private javax.swing.JButton e6;
    private javax.swing.JButton e7;
    private javax.swing.JButton e8;
    private javax.swing.JButton e9;
    private javax.swing.JButton f1;
    private javax.swing.JButton f2;
    private javax.swing.JButton f3;
    private javax.swing.JButton f4;
    private javax.swing.JButton f5;
    private javax.swing.JButton f6;
    private javax.swing.JButton f7;
    private javax.swing.JButton f8;
    private javax.swing.JButton f9;
    private javax.swing.JButton g1;
    private javax.swing.JButton g2;
    private javax.swing.JButton g3;
    private javax.swing.JButton g4;
    private javax.swing.JButton g5;
    private javax.swing.JButton g6;
    private javax.swing.JButton g7;
    private javax.swing.JButton g8;
    private javax.swing.JButton g9;
    private javax.swing.JButton h1;
    private javax.swing.JButton h2;
    private javax.swing.JButton h3;
    private javax.swing.JButton h4;
    private javax.swing.JButton h5;
    private javax.swing.JButton h6;
    private javax.swing.JButton h7;
    private javax.swing.JButton h8;
    private javax.swing.JButton h9;
    private javax.swing.JButton i1;
    private javax.swing.JButton i2;
    private javax.swing.JButton i3;
    private javax.swing.JButton i4;
    private javax.swing.JButton i5;
    private javax.swing.JButton i6;
    private javax.swing.JButton i7;
    private javax.swing.JButton i8;
    private javax.swing.JButton i9;
    private javax.swing.JButton jButtonSendChat;
    private javax.swing.JLabel jLabelPlayer;
    private javax.swing.JList jListPlayer;
    private javax.swing.JScrollPane jScrollPane1;
    private javax.swing.JScrollPane jScrollPane3;
    private javax.swing.JTextArea jTextArea1;
    private javax.swing.JTextField jTextFieldChat;
    private javax.swing.JTextField jTextFiledWispName;
    // End of variables declaration//GEN-END:variables
}
